...
/**
 * @title BillHavenEscrowV4
 * @author BillHaven Team
 * @notice Multi-chain P2P Fiat-to-Crypto escrow with MANDATORY Oracle verification
 * @dev SECURITY HARDENED VERSION - No manual payment confirmation bypass
 *
 * V4 SECURITY IMPROVEMENTS:
 * - Oracle verification is MANDATORY for ALL releases (no maker bypass)
 * - makerConfirmPayment() can only confirm AFTER Oracle verification
 * - makerConfirmAndRelease() REMOVED - no instant release possible
 * - Minimum security delay enforced (24 hours for all methods)
 * - Payer can dispute BEFORE release
 * - Auto-release is permissionless after hold period
 *
 * ATTACK VECTORS BLOCKED:
 * - Maker cannot release without webhook confirmation
 * - Maker cannot skip hold period
 * - Maker cannot bypass Oracle verification
 * - All releases require Oracle signature from backend
 *
 * PAYMENT FLOW (V4):
 ...
 * 9. Crypto released to Payer, fee to platform
 */
contract BillHavenEscrowV4 is ReentrancyGuard, Pausable, AccessControl {
    ...
    enum ConfirmationStatus {
        ...
        PAYMENT_VERIFIED,     // 4: Oracle verified payment (REQUIRED for release)
    ...
    struct Bill {
        ...
        bool oracleVerified;         // Oracle verified payment (REQUIRED for release)
        bool makerConfirmed;         // Maker acknowledged (optional, no longer bypasses)

        // Timestamps
        uint256 createdAt;
        uint256 fundedAt;
        uint256 paymentSentAt;
        uint256 verifiedAt;          // When Oracle verified payment
    ...
    mapping(bytes32 => bool) public usedSignatures;  // V4 FIX: Prevent signature replay
    ...
    uint256 public constant MIN_SECURITY_DELAY = 24 hours; // V4: Minimum hold period
    ...
    event PayerDisputeRaised(uint256 indexed billId, address indexed payer); // V4: New event
    ...
    error PaymentNotOracleVerified();  // V4: Oracle verification required
    error SecurityDelayRequired();      // V4: Minimum hold period required
    error SignatureAlreadyUsed();       // V4 FIX: Prevent signature replay attack
    ...
    constructor(address _feeWallet) {
        ...
        // V4: All non-crypto payments have minimum 24-hour hold
        holdPeriods[PaymentMethod.CRYPTO] = 0;                    // Instant (crypto-to-crypto)
        holdPeriods[PaymentMethod.CASH_DEPOSIT] = 24 hours;       // V4: Increased from 1 hour
    ...
    /**
     * @notice Oracle verifies payment via webhook signature (ONLY Oracle can call)
     * @dev V4: This is the ONLY way to verify payment and start hold period
    ...
    function verifyPaymentReceived(
        ...
        // V4 FIX: Reduced timestamp window from 1 hour to 5 minutes
        if (_timestamp > block.timestamp) revert InvalidSignature();
        if (_timestamp < block.timestamp - 5 minutes) revert InvalidSignature();

        // V4 FIX: Include chainId and contract address to prevent cross-chain replay
        bytes32 messageHash = keccak256(abi.encodePacked(
            block.chainid,      // V4 FIX: Chain ID prevents cross-chain replay
            address(this),      // V4 FIX: Contract address prevents contract clone replay
            _billId,
            bill.payer,
            bill.billMaker,
            _fiatAmount,
        ...
        // V4 FIX: Track used signatures to prevent replay within valid time window
        ...
        // V4: Calculate hold period with minimum security delay
    ...
    /**
     * @notice Bill Maker acknowledges payment (V4: CANNOT bypass Oracle requirement)
     * @dev V4: Maker can only confirm AFTER Oracle has verified
     * @param _billId The bill ID
     */
    function makerConfirmPayment(uint256 _billId) external whenNotPaused {
        ...
        // V4: CRITICAL - Maker can only confirm if Oracle already verified
        if (!bill.oracleVerified) revert PaymentNotOracleVerified();

        // Must be in verified state (Oracle already confirmed)
    ...
    /**
     * @notice V4: REMOVED - Maker can NO LONGER skip hold period
     * @dev This function exists only to give a helpful error message
     * @param _billId The bill ID
     */
    function makerConfirmAndRelease(uint256 _billId) external view {
        // V4: Always revert - no bypass allowed
        if (bills[_billId].billMaker != address(0)) {
            revert SecurityDelayRequired();
        }
        revert BillNotFound();
    }

    /**
     * @notice V4: Auto-release after hold period (PERMISSIONLESS - anyone can call)
     * @dev This is the primary release mechanism in V4
     * @param _billId The bill ID
     */
    function autoReleaseAfterHoldPeriod(uint256 _billId) external nonReentrant whenNotPaused {
        Bill storage bill = bills[_billId];
    ...
    /**
     * @notice V4: Payer can raise dispute BEFORE release
     * @dev Allows payer to block release if something is wrong
     * @param _billId The bill ID
     */
    function payerDisputeBeforeRelease(uint256 _billId) external whenNotPaused {
        Bill storage bill = bills[_billId];

        if (msg.sender != bill.payer) revert NotBillPayer();

        // Can only dispute before release
        ...
        // Must have claimed the bill
        if (bill.payer == address(0)) revert BillNotClaimed();
        ...
        emit PayerDisputeRaised(_billId, msg.sender);
        emit BillDisputed(_billId, msg.sender);
    }

    /**
     * @notice Legacy release function - redirects to auto-release
     * @param _billId The bill ID
     */
    ...
    /**
     * @notice Raise a dispute (both parties can call)
     * @param _billId The bill ID
     */
    ...
    function resolveDispute(
        ...
        if (_releaseToPayer && bill.payer != address(0)) {
            // V4: Arbitrator can release even without Oracle in disputes
            _releaseFundsArbitration(_billId);
        } else {
    ...
    /**
     * @dev V4: Release funds to payer (requires Oracle verification)
     */
    function _releaseFunds(uint256 _billId) internal {
        Bill storage bill = bills[_billId];

        // V4: CRITICAL SECURITY CHECK - MUST be Oracle verified
        if (!bill.oracleVerified) revert PaymentNotOracleVerified();
    ...
    /**
     * @dev V4: Arbitration release (bypasses Oracle check for dispute resolution)
     */
    function _releaseFundsArbitration(uint256 _billId) internal {
        Bill storage bill = bills[_billId];
    ...
    /**
     * @notice V4: Check if bill can be released (requires Oracle verification)
     */
    function canRelease(uint256 _billId) external view returns (bool, string memory) {
        ...
        if (!bill.oracleVerified) {
            return (false, "Oracle verification required");
        }
        if (bill.status != ConfirmationStatus.PAYMENT_VERIFIED &&
            bill.status != ConfirmationStatus.HOLD_COMPLETE) {
    ...
    /**
     * @notice Get minimum security delay
     */
    function getMinSecurityDelay() external pure returns (uint256) {
        return MIN_SECURITY_DELAY;
    }
