/**
 * BillHaven Backend Server - PRODUCTION-READY SECURE VERSION
 * Handles webhooks for Stripe and OpenNode payment confirmations
 *
 * SECURITY IMPROVEMENTS:
 * - Fixed CORS wildcard (production-only URLs)
 * - Added input validation for all API endpoints
 * - Removed error details in production
 * - Added proxy trust for Railway/Render
 * - Added global error handler
 * - Improved logging with environment-based filtering
 * - Added cleanup for rate limit map
 */
...
function validateLightningInvoice(body) {
  const { amount, currency, billId } = body;
  ...
  try {
    ...
    if (error) {
      ...
      status.errors.supabase = {
        ...
        code: error.code
        // SECURITY FIX: Removed stack trace exposure
      };
    } else {
    ...
    status.errors.supabase = {
      ...
      // SECURITY FIX: No stack traces in production
    };
  }
...
// Create Stripe PaymentIntent - with rate limiting and input validation
...
// Create OpenNode Lightning invoice - with rate limiting and input validation
app.post('/api/create-lightning-invoice', rateLimit, async (req, res) => {
  try {
    // SECURITY FIX: Validate input
    const validation = validateLightningInvoice(req.body);
    if (!validation.valid) {
      return res.status(400).json({ error: validation.error });
    }
    ...
    // SECURITY FIX: Don't expose error details in production
    res.status(500).json({
      error: IS_PRODUCTION ? 'Invoice creation error' : error.message
    });
  }
});
...
// ==========================================
// GLOBAL ERROR HANDLER - SECURITY FIX
// ==========================================
